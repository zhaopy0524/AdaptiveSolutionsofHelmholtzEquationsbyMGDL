...
def multigrade_dnn_model_grade_1(trained_variable, data, layers_dims, opt_parameter, stop_criterion, max_learning_rate, min_learning_rate, epochs, activation):
    ...
    solution = spla.spsolve(data['AA'], data['BB'])

    index = 0
    for i in range(1, data['ntrain'] - 1):
        for j in range(1, data['ntrain'] - 1):
            data['u'][i, j] = solution[index]
            index += 1
    train_Y = np.array([data['u'].flatten()])
