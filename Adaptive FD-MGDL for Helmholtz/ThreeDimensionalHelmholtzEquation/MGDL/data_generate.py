...
def generate_data(k):
    ...
    def right(k):
        ...
        for i in range(0,q.shape[1]):
            if (i+1) % m ==0:
                q[0][i] -= np.sin(np.sqrt(3)/3*k)*np.sin(np.sqrt(3)/3*k*train_X[0][i])*np.sin(np.sqrt(3)/3*k*train_X[1][i])
            
            if (i+1) % mm ==0:
                for j in range(0,m):
                    q[0][i-j] -= np.sin(np.sqrt(3)/3*k)*np.sin(np.sqrt(3)/3*k*train_X[0][i-j])*np.sin(np.sqrt(3)/3*k*train_X[2][i-j])
            
            if (i+1) % n ==0:
                for l in range(0,mm):
                    q[0][i-l] -= np.sin(np.sqrt(3)/3*k)*np.sin(np.sqrt(3)/3*k*train_X[1][i-l])*np.sin(np.sqrt(3)/3*k*train_X[2][i-l])
        return q
